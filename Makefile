...
.PHONY: images
images:
	echo docker build . -f Dockerfile -t $(REGISTRY)/$(IMAGE_NAME):$(BUILD_TAG)
	echo docker build . -f Dockerfile.orchestrator -t $(REGISTRY)/$(ORCHESTRATOR_IMAGE_NAME):$(BUILD_TAG)
	echo docker build . -f Dockerfile.sidecar -t $(REGISTRY)/$(SIDECAR_MYSQL57_IMAGE_NAME):$(BUILD_TAG)
	echo docker build . -f Dockerfile.sidecar --build-arg XTRABACKUP_PKG=percona-xtrabackup-80 \
