...
import (
	...
	"github.com/presslabs/mysql-operator/pkg/sync"
	"github.com/presslabs/mysql-operator/pkg/sync/mysqlcluster"
)
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &ReconcileMysqlCluster{Client: mgr.GetClient(), scheme: mgr.GetScheme(), recorder: mgr.GetRecorder(controllerName)}
}
...
func (r *ReconcileMysqlCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	syncers := []sync.Syncer{
		mysqlcluster.NewSyncConfigMap(cluster),
		mysqlcluster.SyncSecret(cluster),
		mysqlcluster.SyncDeployment(cluster),
		mysqlcluster.SyncPDB(cluster),
		mysqlcluster.SyncServices(cluster),

		mysqlcluster.SyncMasterEndpoints(cluster),
		mysqlcluster.SyncHealthyEndpoints(cluster),

		// TODO: ...
		//mysqlcluster.SyncPodsLabels(cluster),
	}
